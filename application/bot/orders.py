...
def _to_the_address(chat_id, language):
    ...
    # address_keyboard = keyboards.get_keyboard('remove')
    address_keyboard = keyboards.get_keyboard('go_back', language)
...
def _to_the_delivery_time(chat_id, language) -> None:
    time_message = strings.get_string('order.delivery_time', language)
    back_keyboard = keyboards.get_keyboard('go_back', language)
    bot.send_message(chat_id, time_message, parse_mode='HTML', reply_markup=back_keyboard)
    bot.register_next_step_handler_by_chat_id(chat_id, delivery_time_processor)
...
def shipping_method_processor(message: Message):
    ...
    user_info = userservice.get_user_by_telegram_id(user_id)

    def phone_number():
        _to_the_phone_number(chat_id, language, user_info)
    ...
    if strings.get_string('go_to_menu', language) in message.text:
        ...
        _to_the_address(chat_id, language)
        # _to_the_phone_number(chat_id, language, user_id)
    else:
        error()
        return
    ...
    if message.contact is not None:
        ...
        if message.text is None:
            ...
            if strings.get_string('go_back', language) in message.text:
                # _to_the_address(chat_id, language)
                _to_the_delivery_time(chat_id, language)
...
def delivery_time_processor(message: Message):
    chat_id = message.chat.id
    user_id = message.from_user.id
    language = userservice.get_user_language(user_id)

    def error():
        error_msg = strings.get_string('order.delivery_time_error')
        bot.send_message(chat_id, error_msg, parse_mode='HTML')
        bot.register_next_step_handler_by_chat_id(chat_id, delivery_time_processor)

    def error1():
        error_msg = strings.get_string('order.delivery_time_error1')
        bot.send_message(chat_id, error_msg, parse_mode='HTML')
        bot.register_next_step_handler_by_chat_id(chat_id, delivery_time_processor)
    ...
    if strings.get_string('go_back', language) in message.text:
        _to_the_address(chat_id, language)
        return
    match = re.match(r'^([01]\d|2[0-3])[.:_][0-5]\d$', message.text)
    if match is None:
        error()
        return
    now = datetime.now(tz).time()
    d_time = match.group()
    d_time = f"{d_time[:2]}:{d_time[3:]}"
    c_time = datetime.strptime(d_time, '%H:%M').time()
    if now < c_time:
        current_order = orderservice.set_delivery_time(user_id, d_time)
        phone_number()
        return
    error1()
