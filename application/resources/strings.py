...
def from_cart_items(cart_items, language, total) -> str:
    ...
    for cart_item in cart_items:
        ...
        if language == 'uz':
            dish_item = cart_str_item.format(counter=counter,
                                             ...
                                             sum=_format_number(
                                                 cart_item.count * cart_item.dish.price))
    ...
    cart_contains += "\n<b>{}</b>: {} {}".format(get_string('cart.summary', language),
                                                 _format_number(total), # _format_number(total * currency_value),
...
def from_order(order: Order, language: str, total: int) -> str:
    ...
    order_content += '<b>{time}:</b> {time_value}\n'.format(time=get_string('time', language),
                                                            time_value=order.delivery_time)
...
def from_order_notification(order: Order, total_sum):
    ...
    order_content += '<b>Время доставки:</b> {}\n'.format(order.delivery_time)
...
def from_order_items_to_labeled_prices(order_items: List[OrderItem], language) -> List[LabeledPrice]:
    currency_value = settings.get_currency_value()
    return [LabeledPrice(from_dish_name(oi.dish, language) + ' x ' + str(oi.count), int(oi.count * oi.dish.price )) for oi in order_items]
